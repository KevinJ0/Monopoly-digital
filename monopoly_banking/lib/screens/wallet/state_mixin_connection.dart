...
mixin _WalletConnection on State<WalletScreen> {
  ...
  void _listenToBankruptcy() {
    ...
    // No llamar listener inmediato si la sesión ya estaba hecha (reconexión evita
    // abrir bancarrota automática al restaurar estado desde Hive).
  ...
  Future<void> _connectToBank(String host, {int port = 8080}) async {
    ...
    if (P2PService().wsTransport.clientConnectedNotifier.value) {
      debugPrint('[┊] CONNECT WS already connected, closing client first');
