...
class WalletController extends ChangeNotifier {
  ...
  void investLocal(double amount, int passes) {
    final session = _session;
    if (session == null || session.isBankrupt) {
      throw Exception('No hay sesión activa o estás en bancarrota.');
    }
    if (amount <= 0 || passes < 1 || passes > 5) {
      throw Exception('Monto o pases inválidos.');
    }
    if (session.balance - amount <= 0) {
      throw Exception('No puedes invertir todo tu saldo. Debe quedar al menos \$1.');
    }
    if (session.vaultInvestedAmount > 0) {
      throw Exception('Ya tienes una inversión activa. Retírala primero.');
    }
    session.balance -= amount;
    session.vaultInvestedAmount = amount;
    session.vaultGeneratedAmount = 0;
  ...
  double withdrawLocal() {
    final session = _session;
    if (session == null || session.vaultInvestedAmount <= 0) {
      throw Exception('No hay inversión activa para retirar.');
    }
    if (session.vaultCurrentPasses < session.vaultTargetPasses) {
      throw Exception(
        'Aún no has completado los pases por GO. Llevas ${session.vaultCurrentPasses}/${session.vaultTargetPasses}.',
      );
    }
    final invested = session.vaultInvestedAmount;
    final generated = session.vaultGeneratedAmount;
    final total = invested + generated;
    session.balance += total;
    session.vaultInvestedAmount = 0;
    session.vaultGeneratedAmount = 0;
  ...
  void passGoLocal() {
    final session = _session;
    if (session == null || session.isBankrupt) return;
    session.balance += kPassGoAmount;
    if (session.vaultInvestedAmount > 0 &&
        session.vaultCurrentPasses < session.vaultTargetPasses) {
      session.vaultCurrentPasses += 1;
      const rates = [0.05, 0.07, 0.10, 0.12, 0.15];
      final rate = rates[session.vaultTargetPasses - 1];
      final interest = session.vaultInvestedAmount * rate;
      session.vaultGeneratedAmount += interest;
    }
    session.save();
    _updateVaultNotifiers(session);
    _persistTx(
